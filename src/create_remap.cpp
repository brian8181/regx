...
std::map<string, string>& create_map(string pattern, string s, std::map<string, string>& map);
...
int main(int argc, char* argv[]) 
{
    ...
    // replace_all(input_pattern_str, ".", "\\.");
    // const regex TAG_EXP(TAG_MATCH_EXP_STR);
    // const string REPLACE_EXP_STR 
    //     = "^" + regex_replace(input_pattern_str, TAG_EXP, "(" + SPECIAL_CHARS_STR + "*)" ) + "$";
    ...
    // int idx = 1;
    // auto begin = sregex_iterator(input_pattern_str.begin(), input_pattern_str.end(), TAG_EXP);
...
std::map<string, string>& create_map(string pattern, string s, std::map<string, string>& map)
{
    ...
    // create smatch
    std::smatch sm;    
    std::regex_match (s, sm, REPLACE_EXP);
    ...
    {
        smatch match = *i;
        map.insert(std::make_pair(match.str(1), sm.str(idx)));
...
/*
 ./create_remap "<track>. <artist>-<album>-<title>.<type>" \
 "10. The Rolling Stones-Exile On Main Street-Brown Sugar.mp3" \
 "<track>: <title>.<type"
